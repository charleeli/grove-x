...
private:
    ...
    int     push_port_          ;
    int     push_io_threads_num_;
    int     push_task_threads_num_;
...
public:
    CouponConfig()
    {
        ...
        push_port_      = 19000;
        push_io_threads_num_ = 1;
        push_task_threads_num_ = 2;
    ...
    void Parse(string conf)
    {
        ...
        redis_ip_               = jh.Root["coupon_redis_server"]["ip"].GetString();
        redis_port_             = jh.Root["coupon_redis_server"]["port"].GetInt();
        redis_timeout_          = jh.Root["coupon_redis_server"]["timeout"].GetInt();

        mysql_ip_               = jh.Root["coupon_mysql_server"]["ip"].GetString();
        mysql_port_             = jh.Root["coupon_mysql_server"]["port"].GetInt();
        mysql_user_             = jh.Root["coupon_mysql_server"]["user"].GetString();
        mysql_passwd_           = jh.Root["coupon_mysql_server"]["password"].GetString();
        mysql_database_         = jh.Root["coupon_mysql_server"]["dbname"].GetString();

        log_path_               = jh.Root["coupon_log_path"]["path"].GetString();

        push_port_              = jh.Root["coupon_push_server"]["port"].GetInt();
        push_io_threads_num_    = jh.Root["coupon_push_server"]["ioThreadsNum"].GetInt();
        push_task_threads_num_  = jh.Root["coupon_push_server"]["taskThreadsNum"].GetInt();

        ec_port_                = jh.Root["coupon_ec_server"]["port"].GetInt();
        ec_io_threads_num_      = jh.Root["coupon_ec_server"]["ioThreadsNum"].GetInt();
        ec_task_threads_num_    = jh.Root["coupon_ec_server"]["taskThreadsNum"].GetInt();

        admin_port_             = jh.Root["coupon_admin_server"]["port"].GetInt();
    }
    ...
    int getPushPort(){return this->push_port_;}
    int getPushIoThreadsNum(){
        if (this->push_io_threads_num_ <= 0)
            this->push_io_threads_num_ = 1;

        return this->push_io_threads_num_;
    }
    int getPushTaskThreadsNum(){
        if(this->push_task_threads_num_ <= 0){
            this->push_task_threads_num_ = 1;
        }

        return this->push_task_threads_num_;
    }
    ...
    void Print(){
        ...
        cout<<"push_io_threads_num = "<< getPushIoThreadsNum()<<endl;
        cout<<"push_task_threads_num = "<< getPushTaskThreadsNum()<<endl;
